...
ESX  = nil
local PlayerData    = {}
local duzykutaspedala					= false		
local jeszczewiekszykutaspedala				= false		
...
RegisterNetEvent('animacjecuff:fiveguardkick')
AddEventHandler('animacjecuff:fiveguardkick', function(target)
	jeszczewiekszykutaspedala = true
	...
	jeszczewiekszykutaspedala = false
end)

RegisterNetEvent('animacjecuff:fiveguardban')
AddEventHandler('animacjecuff:fiveguardban', function()
	local playerPed = GetPlayerPed(-1)
	RequestAnimDict(SectionAnimation)
	while not HasAnimDictLoaded(SectionAnimation) do
		Citizen.Wait(10)
